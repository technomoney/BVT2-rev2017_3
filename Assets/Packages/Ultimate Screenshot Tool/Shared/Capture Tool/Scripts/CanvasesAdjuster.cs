...
namespace TRS.CaptureTool
{
    public static class CanvasesAdjuster
    {
        ...
        public static void Setup()
        {
            ...
            List<Canvas> allCanvasesList = new List<Canvas>();
        ...
        static float ClosestZPositionOffset(Canvas canvas)
        {
            float minValue = 0f;
            foreach (Transform canvasTransform in canvas.GetComponentsInChildren<Transform>())
            {
                float canvasElementDistance = canvas.transform.InverseTransformPoint(canvasTransform.position).z;//canvasTransform.position.z;
                if (canvasElementDistance < minValue)
                    minValue = canvasElementDistance;
            }
