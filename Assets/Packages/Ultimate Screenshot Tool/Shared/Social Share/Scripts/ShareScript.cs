...
#pragma warning restore 0414
...
#if UNITY_EDITOR
#if UNITY_ANDROID
            const string relativePathToAndroidManifest = "Plugins/Android/AndroidManifest.xml";
            string separatorString = System.IO.Path.DirectorySeparatorChar.ToString();
            string androidManifestPath = Application.dataPath + separatorString + string.Join(separatorString, relativePathToAndroidManifest.Split('/'));
            if (!System.IO.File.Exists(androidManifestPath))
                Debug.LogWarning("Native Share Error: No existing AndroidManifest at " + relativePathToAndroidManifest + ". Manifest must be customized as described in the Project Setup section of the ReadMe for Share Sheet to work.");
            else
            {
                string androidManifest = System.IO.File.ReadAllText(androidManifestPath);
                if (!androidManifest.Contains("com.yasirkula.unity.UnitySSContentProvider"))
                    Debug.LogWarning("Native Share Error: Manifest must be customized as described in the Project Setup section of the ReadMe for Share Sheet to work.");
            }
#endif

            CaptureScript captureScript = GetComponentInParent<CaptureScript>();
            if (captureScript == null)
                captureScript = GetComponentInChildren<CaptureScript>();
            if (captureScript != null)
                captureScript.SetShareScript(this);
#endif
        }
        ...
        public void UploadToImgur()
        {
            ...
            if (imgurTags.Length > 0)
                text += " " + imgurTagsText;
            APIShare.UploadToImgur(config, mediaToUploadPath, text, title, imgurAlbum, SetNewMediaUrls);
        }
