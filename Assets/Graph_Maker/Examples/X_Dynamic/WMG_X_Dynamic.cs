...
//		}
	...
	IEnumerator startTests() {
		...
		// since font size is an integer instead of float, enable font scaling for smoother looking resize animation
		// however the font can look blurry when overscaled, in which case you can leave this disabled.
		graph.resizingChangesFontScaleInsteadOfFontSize = true;
		if (!noTestDelay) yield return new WaitForSeconds(testInterval);
		StartCoroutine(sizeTests());
		if (!noTestDelay) yield return new WaitForSeconds(testInterval * 3);
		graph.resizeEnabled = false;
		graph.resizeProperties = (WMG_Axis_Graph.ResizeProperties)0;
		graph.resizingChangesFontScaleInsteadOfFontSize = false;
		yield return new WaitForSeconds(testGroupInterval);
	...
	IEnumerator autofitTests() {
		...
		string s3 = "This is a lonnnng string";
		...
		graph.yAxis.AxisLabelRotation = 45;
		...
		graph.yAxis.AxisLabelRotation = 0;
	...
	IEnumerator seriesTests() {
		...
		s1.pointValues.SetList(WMG_Util.GenCircular(s1.pointValues.Count, graph.xAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f-2));
		s1.connectFirstToLast = true;

		// Triangle
		if (!noTestDelay) yield return waitTime;
		s1.pointValues.SetList(WMG_Util.GenCircular(3, graph.xAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f-2));

		// Triangle rotated 90 degrees, auto animations enabled for awesomeness
		graph.autoAnimationsEnabled = true;
		if (!noTestDelay) yield return waitTime;
		s1.pointValues.SetList(WMG_Util.GenCircular(3, graph.xAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f-2, 90));

		if (!noTestDelay) yield return waitTime;
		graph.autoAnimationsEnabled = false;
		s1.pointValues.SetList(WMG_Util.GenCircular(50, graph.xAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f, graph.yAxis.AxisMaxValue/2f-2));
		s1.linePadding = 0;
		...
		if (!noTestDelay) yield return waitTime;
		s1.pointValues.SetList(WMG_Util.GenRandomXY(50, graph.xAxis.AxisMinValue, graph.xAxis.AxisMaxValue, graph.yAxis.AxisMinValue, graph.yAxis.AxisMaxValue));
	...
	IEnumerator dynamicDataPopulationViaReflectionTests() {
		...
		List<Vector2> randomData = WMG_Util.GenRandomY(graph.groups.Count, 1, graph.groups.Count, graph.yAxis.AxisMinValue, graph.yAxis.AxisMaxValue);
		...
		randomData = WMG_Util.GenRandomY(graph.groups.Count, 1, graph.groups.Count, graph.yAxis.AxisMinValue, graph.yAxis.AxisMaxValue);
		for (int i = 0; i < graph.groups.Count; i++) {
	...
	IEnumerator hideShowTests() {
		...
		if (!noTestDelay) yield return waitTime;
		graph.xAxis.hideAxisLine = true;

		if (!noTestDelay) yield return waitTime;
		graph.yAxis.hideAxisLine = true;
		...
		graph.yAxis.HideAxisArrowTopRight = false;
		graph.xAxis.HideAxisArrowTopRight = false;
		graph.xAxis.hideAxisLine = false;
		graph.yAxis.hideAxisLine = false;
		s1.hideLines = false;
		s2.hideLines = false;
		s1.hidePoints = false;
		s2.hidePoints = false;
	...
	IEnumerator axesTests() {
		...
		WMG_Anim.animVec2(()=> graph.theOrigin, x=> graph.theOrigin = x, animDuration, new Vector2(0, 0), Ease.Linear);
	...
	void addSeriesWithRandomData() {
		...
		series.pointValues.SetList(WMG_Util.GenRandomY(graph.groups.Count, 1, graph.groups.Count, graph.yAxis.AxisMinValue, graph.yAxis.AxisMaxValue));
